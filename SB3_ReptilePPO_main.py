...
from MetaPPO.MakeEnv import create_reach_env
...
if __name__ == "__main__":
    ...
    for meta_iter in range(META_ITERATIONS):
        ...
        for key in current_meta_params["policy"]:
            ...
            avg_update_direction[key] = (
                avg_adapted_param - current_meta_params["policy"][key]
            )

        updated_meta_params = deepcopy(current_meta_params)
        for key in updated_meta_params["policy"]:
            updated_meta_params["policy"][key] = (
                current_meta_params["policy"][key] + META_LR * avg_update_direction[key]
            )
        ...
        if meta_iter % 1 == 0:
            ...
            print(
                f"Meta-Iteration: {meta_iter}/{META_ITERATIONS} | Avg Eval Reward: {avg_eval_reward:.2f}"
            )
            writer.add_scalar("Evaluation/Average_Reward", avg_eval_reward, meta_iter)
            if model_save:
                meta_model.save(
                    os.path.join(LOG_DIR, f"maml_ppo_model_{meta_iter}_steps")
                )
