...
class MAML:
    def __init__(
        ...
        self.value_function = ValueNetwork(state_dim, policy_kwargs)
        self.value_optimizer = optim.Adam(self.value_function.parameters(), lr=meta_lr)
    ...
    def meta_train(
        ...
        for iteration in range(num_meta_iterations):
            ...
            # Update value function using all query trajectories
            self._update_value_function(all_data)
    ...
    def _update_value_function(self, data_list):
        all_data = self._flatten_trajectories(data_list)
        values = self.value_function(all_data["states"]).squeeze(-1)
        value_loss = F.mse_loss(values, all_data["returns"])
    ...
    def _ppo_loss(
        ...
        values = self.value_function(data["states"])
    ...
    def _collect_trajectory(
        ...
        with torch.no_grad():
            state_t = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
            last_value = self.value_function(state_t).squeeze(0).cpu()
