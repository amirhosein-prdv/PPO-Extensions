...
from .Networks import LinearValueNetwork, PolicyNetwork
...
class MAML:
    def __init__(
        ...
        self.base_policy = PolicyNetwork(state_dim, action_dim, policy_kwargs)
        self.meta_optimizer = optim.SGD(self.base_policy.parameters(), lr=meta_lr)

        # Value function
        self.value_function = LinearValueNetwork(state_dim)
    ...
    def meta_train(
        ...
        for iteration in range(num_meta_iterations):
            ...
            for task_idx, task_env in enumerate(task_envs):
                ...
                data = self._collect_trajectories(task_env, adapted_params)
    ...
    def _inner_update(self, task_env, base_params):
        ...
        for _ in range(self.inner_steps):
            ...
            data = self._collect_trajectories(task_env, fast_params)
    ...
    def _ppo_loss(
        ...
        new_logprobs, entropy = self.base_policy.evaluate_action(
            data["states"], data["actions"], params
        )
        values = self.value_function(data["states"]).to(self.device)
        ...
        if looptype == "inner":
            total_loss = (
                policy_loss + 0 * value_loss - self.inner_ent_coef * entropy_loss
            )
        elif looptype == "outer":
            total_loss = (
                policy_loss + 0 * value_loss - self.meta_ent_coef * entropy_loss
            )
        else:
    ...
    def _collect_trajectory(
        ...
        for _ in range(max_steps):
            ...
            action, logprob = self.base_policy(state_t, policy_params)
            value = self.value_function(state_t)
        ...
        discount_returns = torch.tensor(
            [
                np.sum(rewards[i:] * (self.gamma ** np.arange(i, len(rewards))))
                for i in range(len(rewards))
            ],
            dtype=torch.float32,
        )
        if value_fit:
            self.value_function.fit(torch.cat(states), discount_returns)
        values = self.value_function(torch.cat(states)).detach().cpu()

        # Bootstrap value for the last state
        with torch.no_grad():
            state_t = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
            last_value = self.value_function(state_t).squeeze(0)
    ...
    def _collect_trajectories(
        ...
        task_env: gym.Env,
        fast_params=None,
        traj_per_task: int = None,
        max_steps: int = None,
    ) -> Dict[str, torch.Tensor]:
        """
        Collect multiple trajectories from a task environment and flatten them.

        Args:
            task_env: Environment for the current task
            fast_params: Fast policy parameters for adaptation
            traj_per_task: Number of trajectories to collect (defaults to self.traj_per_task)
            max_steps: Maximum steps per trajectory (defaults to self.max_steps)
            value_fit: Whether to fit value function during collection

        Returns:
            Flattened trajectory data ready for policy updates
        """
        if traj_per_task is None:
            traj_per_task = self.traj_per_task
        if max_steps is None:
            max_steps = self.max_steps
        ...
        for _ in range(traj_per_task):
            ...
            for _ in range(max_steps):
                state_t = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
                action, logprob = self.base_policy(state_t, fast_params)
                value = self.value_function(state_t)

                next_state, reward, terminated, truncated, _ = task_env.step(
                    action.detach().cpu().numpy().squeeze(0)
                )
                done = terminated or truncated
            ...
            # Compute discount returns for value fitting
            discount_returns = torch.tensor(
                [
                    np.sum(rewards[i:] * (self.gamma ** np.arange(i, len(rewards))))
                    for i in range(len(rewards))
            ...
            # Store trajectory data and fitting data
            all_trajectories.append(
                {
                    ...
                    "rewards": torch.tensor(rewards, dtype=torch.float32),
                    "dones": torch.tensor(dones, dtype=torch.float32),
                    "last_state": torch.tensor(state, dtype=torch.float32),
                }
            )

            all_states_for_fit.append(torch.cat(states))
            all_returns_for_fit.append(discount_returns)

        # Fit value function using all trajectories
        all_states_fit = torch.cat(all_states_for_fit)
        all_returns_fit = torch.cat(all_returns_for_fit)
        self.value_function.fit(all_states_fit, all_returns_fit)

        # Now compute advantages for all trajectories
        all_values = []
        all_advantages = []
        all_returns = []

        for traj in all_trajectories:
            # Get value predictions for all states in this trajectory
            values_tensor = self.value_function(traj["states"]).detach().cpu()

            # Bootstrap value for the last state
            with torch.no_grad():
                last_value = self.value_function(
                    traj["last_state"].unsqueeze(0)
                ).squeeze(0)
        ...
        states = torch.cat([traj["states"] for traj in all_trajectories]).to(
            self.device
        )
        actions = torch.cat([traj["actions"] for traj in all_trajectories]).to(
            self.device
        )
        advantages = torch.cat(all_advantages).to(self.device)
        returns = torch.cat(all_returns).to(self.device)
        old_logprobs = torch.cat([traj["logprobs"] for traj in all_trajectories]).to(
            self.device
        )
        old_values = torch.cat(all_values).to(self.device)
